...
Observable.prototype = {
	...
	init: function Observable_init() {
		DEBUG && console && console.log('Observable', 'init', arguments);
	...
	on: function Observable_on(eventList, callback) {
		DEBUG && console && console.log('Observable', 'on', arguments);
	...
	off: function Observable_off(eventList, callback) {
		DEBUG && console && console.log('Observable', 'off', arguments);
		...
		for (var i = 0, l = eventList.length; i < l; i++) {
			...
			if (!(event in this.eventsMap) || this.eventsMap[event].length === 0) {
				console && console.error('Observable', 'off', 'list of event handlers is empty', event);
				return false;
			}
			
			var callbackIdx = this.eventsMap[event].indexOf(callback);
			if (callbackIdx === -1) {
				console && console.error('Observable', 'off', 'no such event handler', event, callback);
				return false;
			}
	...
	trigger: function Observable_trigger(eventList, data) {
		DEBUG && console && console.log('Observable', 'trigger', arguments);
		...
		for (var i = 0, l = eventList.length; i < l; i++) {
			...
			for (var i = 0, l = this.eventsMap[event].length; i < l; i++) {
				...
				try {
					...
					console && console.error('Observable', 'trigger', 'event handler run error', err);
				}
			}
		}
